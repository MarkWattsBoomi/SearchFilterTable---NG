...
export class ColumnRules {
    static async parse(ruleStr: string, parent: SFT): Promise<Map<string, ColumnRule>> {
        ...
        while (match = RegExp(/^{{([^}]*)}}/).exec(ruleStr)) {
            ...
            let val: FlowValue = await parent.component.getValue(fldElements[0]);            
...
export class ColumnRule {
    ...
    generateColumnContent(value: FlowObjectDataProperty, row: FlowObjectData, sft?: SFT, editable?: boolean, onChange?: (e: any) => void, onKeyDown?: (e: any) => void): any {
        const style: React.CSSProperties = {};
            ...
            switch (this.mode) {
                ...
                case "currency":
                    ...
                    if(value.value !== null && !isNaN(value.value)) {
                        ...
                        amt = formatter.format(value.value as number);
                    }
                    content = (
